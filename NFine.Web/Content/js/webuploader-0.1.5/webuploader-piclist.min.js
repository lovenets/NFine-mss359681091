jQuery(function () {
        ...
        if (e !== k) {
            switch (c.removeClass("state-" + k), c.addClass("state-" + e), k = e) {
                ...
                case "confirm":
                    ...
                    break;
                case "finish":
                    a = n.getStats(), a.successNum ? $.modalMsg('上传成功', 'success') : (k = "done", location.reload())
            }
            i()
        }
    }
    ...
    if (!WebUploader.Uploader.support()) throw $.modalMsg("Web Uploader 不支持您的浏览器！如果你使用的是IE浏览器，请尝试升级 flash 播放器: ", 'warning'), new Error("WebUploader does not support the browser you are using.");
    ...
    n = WebUploader.create({
        ...
        disableGlobalDnd: !0,
        chunked: !0,
        server: "/Canchong/PicList/UpLoadProcess",
        formData: { "param_uploader": o("#param_uploader").val() },
        fileNumLimit: 300,
        ...
        parent.$("#F_Nick").val(response.F_Nick);
        parent.$("#F_FilePath").val(response.F_FilePath);
        parent.$("#F_FullName").val(response.F_FullName);
        parent.$("#F_FileSize").val(response.F_FileSize);
        parent.$("#F_ThumbnailPath").val(response.F_ThumbnailPath);
